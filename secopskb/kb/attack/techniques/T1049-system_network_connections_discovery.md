...
Utilities and commands that acquire this information include [[netstat|netstat]], "net use," and "net session" with [[net|Net]]. In Mac and Linux, [[netstat|netstat]] and `lsof` can be used to list current connections. `who -a` and `w` can be used to show which users are currently logged in, similar to "net session". Additionally, built-in features native to network devices and [[T1059-command_and_scripting_interpreter#^t1059008-network-device-cli|T1059.008: Network Device CLI]] may be used (e.g. `show ip sockets`, `show tcp brief`).(Citation: US-CERT-TA18-106A) On ESXi servers, the command `esxi network ip connection list` can be used to list active network connections.(Citation: Sygnia ESXi Ransomware 2025)

## Workspace

- [[notes/attack/techniques/T1049-system_network_connections_discovery-note|Open workspace note]]

![[notes/attack/techniques/T1049-system_network_connections_discovery-note]]
