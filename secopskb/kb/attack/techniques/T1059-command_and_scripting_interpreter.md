...
- [[workspaces/attack/techniques/T1059-command_and_scripting_interpreter-note|Open workspace note]]
...
A number of PowerShell-based offensive testing tools are available, including [[empire|Empire (S0363)]],  [[powersploit|PowerSploit (S0194)]], [[poshc2|PoshC2 (S0378)]], and PSAttack.(Citation: Github PSAttack)
...
Adversaries may abuse the Windows command shell for execution. The Windows command shell ([[cmd|cmd (S0106)]]) is the primary command prompt on Windows systems. The Windows command prompt can be used to control almost any aspect of a system, with various permission levels required for different subsets of commands. The command prompt can be invoked remotely via [[T1021-remote_services|T1021: Remote Services]] such as [[T1021-remote_services#^t1021004-ssh|T1021.004: SSH]].(Citation: SSH in Windows)

Batch files (ex: .bat or .cmd) also provide the shell with a list of sequential commands to run, as well as normal scripting operations such as conditionals and loops. Common uses of batch files include long or repetitive tasks, or the need to run the same set of commands on multiple systems.

Adversaries may leverage [[cmd|cmd (S0106)]] to execute various commands and payloads. Common uses include [[cmd|cmd (S0106)]] to execute a single command, or abusing [[cmd|cmd (S0106)]] interactively with input and output forwarded over a command and control channel.
